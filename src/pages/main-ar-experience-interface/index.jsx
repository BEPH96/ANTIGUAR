...
const MainARExperienceInterface = () => {
  ...
  const handleMarkerDetected = (marker) => {
    console.log('Marcador detectado:', marker);
    setDetectedMarkers(prev => [...prev, marker]);
    setTrackingQuality('good');
  };

  const handleMarkerLost = (marker) => {
    console.log('Marcador perdido:', marker);
    setDetectedMarkers(prev => prev.filter(m => m.id !== marker.id));
    
    if (detectedMarkers.length <= 1) {
  ...
  const handleSessionToggle = async (shouldStart) => {
    if (shouldStart) {
      ...
      try {
        // Verificar permisos de cámara con getUserMedia
        const stream = await navigator.mediaDevices.getUserMedia({ 
          video: { 
            facingMode: 'environment' // Usar cámara trasera para AR
          } 
        });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative w-full h-screen overflow-hidden">
        ...
        {/* AR Camera View with Gesture Handling (para sesiones no activas) */}
        {!isSessionActive && (
          <ARGestureHandler
            onObjectSelect={handleObjectSelect}
            onObjectMove={handleObjectMove}
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    const newObject = {
                      ...
                      name: `Objeto ${item}`,
                      type: '3d-model',
                      modelUrl: `https://example.com/models/object${item}.gltf`,
                      thumbnail: `https://example.com/thumbnails/object${item}.jpg`
                    };
                    handleObjectSelect(newObject);
                    setShowLibraryPanel(false);
                  }}
                >
                  <span className="text-text-muted">Objeto {item}</span>
